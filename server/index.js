...
mongoose.connect(
  "mongodb://hema_2005:Princess2005@ac-1hyrc92-shard-00-00.jbdjfnk.mongodb.net:27017,ac-1hyrc92-shard-00-01.jbdjfnk.mongodb.net:27017,ac-1hyrc92-shard-00-02.jbdjfnk.mongodb.net:27017/?ssl=true&replicaSet=atlas-14g063-shard-0&authSource=admin&retryWrites=true&w=majority"
)
...
// -------------------------------------------------
// -------------------------------------------------
const myemail = "hemavathy.r0709@gmail.com";
const mypass = "gnrizlqdskpwofhw";
...
// Delete Event API
app.delete("/eventsdata/:id", async (req, res) => {
  const token = req.headers.authorization?.split(" ")[1];
  if (!token) return res.status(401).json({ message: "Unauthorized" });
...
app.delete("/instruments/:id", async (req, res) => {
  ...
  try {
    ...
    if (instrument.userId.toString() !== decoded.userId) {
      return res.status(403).json({
        message: "You cannot delete this instrument",
      });
    }
